export type EditionDefinition = {
  ...
  /** Upcoming partition type shown for jcr-only editions before release. */
  pendingPartitionType?: "cas" | "xr";
  partitionReleaseDate: string;
  impactFactorFile: string;
  /** IF data year in the ShowJCR JCR CSV (e.g. IF(2025) in JCR2025-UTF8.csv). */
  impactFactorYear: number;
  /** Clarivate JCR release date for the edition paired at partition release. */
  impactFactorReleaseDate: string;
};

/**
 * Each partition-backed edition pairs a partition table with the latest JCR
 * impact factor available when that partition table was released.
 */
export const EDITION_DEFINITIONS: EditionDefinition[] = [
  {
    id: "aji-2027",
    label: { zh: "2027 版", en: "2027 Edition" },
    shortLabel: { zh: "JCR 2026", en: "JCR 2026" },
    partitionYear: 2027,
    partitionType: "jcr-only",
    pendingPartitionType: "xr",
    partitionReleaseDate: "",
    impactFactorFile: "JCR2025-UTF8.csv",
    impactFactorYear: 2025,
    impactFactorReleaseDate: "2026-06-17",
  },
  {
    id: "aji-2026",
    label: { zh: "2026 版", en: "2026 Edition" },
    shortLabel: { zh: "JCR 2025 · XR 2026", en: "JCR 2025 · XR 2026" },
  ...
  },
  {
    ...
    shortLabel: { zh: "JCR 2024 · CAS 2025", en: "JCR 2024 · CAS 2025" },
  ...
  {
    id: "aji-2024",
    label: { zh: "2024 版", en: "2024 Edition" },
    shortLabel: { zh: "JCR 2023 · CAS 2023", en: "JCR 2023 · CAS 2023" },
  ...
  },
  {
    ...
    shortLabel: { zh: "JCR 2022 · CAS 2022", en: "JCR 2022 · CAS 2022" },
  ...
  },
  {
    ...
    shortLabel: { zh: "JCR 2021 · CAS 2021", en: "JCR 2021 · CAS 2021" },
    partitionFile: "FQBJCR2021-UTF8.csv",
    partitionYear: 2021,
    partitionType: "cas",
    partitionReleaseDate: "2021-12-20",
    impactFactorFile: "JCR2020-UTF8.csv",
    impactFactorYear: 2020,
    impactFactorReleaseDate: "2021-06-30",
  },
];

export const DEFAULT_EDITION_ID = "aji-2026";

export function getLatestEditionId(
  definitions: EditionDefinition[] = EDITION_DEFINITIONS
): string {
  const partitioned = definitions.filter(
    (definition) => definition.partitionType !== "jcr-only"
  );
  const latest = [...partitioned].sort((a, b) => {
  ...
  if (!latest) {
    throw new Error("No partition-backed edition definitions configured.");
  }
